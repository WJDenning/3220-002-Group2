...
plot_nominal_feature_by_class <- function(df, feature) {
  
  p <- ggplot(df, aes(x=feature, y=len)) + 
  geom_boxplot()
}
...
for (f in nominal_features) {
  ...
  cat("Outlier Visualization\n")
  print(plot_nominal_feature_by_class(train_raw, f))
}
}
```
